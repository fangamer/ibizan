...
{ CONTEXTS, REGEX, STRINGS } = require '../helpers/constants'
...
module.exports =
  init: (controller) ->
    ...
    controller.hears 'bark', 'direct_message,direct_mention,mention,ambient', (bot, msg) ->
      bot.startTyping msg
      bot.reply msg, Logger.random strings.bark

    controller.setupWebserver process.env.PORT, (err, webserver) ->
      webserver.get '/', (req, res) ->
        html = process.env.ORG_NAME + '\'s Ibizan v' + process.env.npm_package_version + ' - ' + moment().format()
        res.send html

  help: [
    'bark - Bark'
    'fetch [thing] - Fetches a thing'
  ]
