...
{ CONTEXTS, REGEX, HEADERS, STRINGS, TIMEZONE } = require '../helpers/constants'
...
module.exports =
  init: (controller) ->
    ...
    isClockChannel = (channel) ->
      chan = controller.storage.channels.get channel
      return channel.name is Organization.clockChannel
    ...
    parse = (bot, msg) ->
      ...
      bot.api.channels.info {channel: msg.channel}, (err, res) ->
        channelname = channel.name
      user = Organization.getUserBySlackName username
      Logger.log "Parsing '#{msg.text}' for @#{user.slack}."
      if canPunchHere channelname
        Logger.addReaction bot, 'clock4', msg
        message = msg.text.replace REGEX.ibizan, ''
        message = message.trim()
        tz = user.timetable.timezone.name or TIMEZONE
        punch = Punch.parse user, message, mode, tz
        if not punch.projects.length and
           isProjectChannel channelname
          project = Organization.getProjectByName channelname
          if project?
        ...
        sendPunch punch, user, msg
      else
        Logger.addReaction bot, 'x', msg
    ...
    sendPunch = (punch, user, msg) ->
      ...
      .then(
        (punch) ->
          ...
          Logger.addReaction bot, 'dog2', msg
          Logger.removeReaction bot, 'clock4', msg
      )
      .catch(
        (err) ->
    ...
    controller.hears [REGEX.modes], CONTEXTS, (bot, msg) ->
      parse bot, msg
