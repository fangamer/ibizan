...
async function onScheduledReportHandler(organization: Organization) {
...
async function onScheduledPayrollHandler(organization: Organization) {
    if (!organization.ready()) {
        console.warn(`Don\'t make scheduled daily report, the ${organization.name} organization isn\'t ready yet.`);
        return;
...
async function onPayrollHandler(bot: botkit.Bot, message: Message) {
...
export default function (controller: botkit.Controller) {
    ...
    const reminderJob = schedule.scheduleJob('0 13 * * 5', () => {
        controller.trigger(EVENTS.payrollWarning, [onScheduledPayrollWarningHandler]);
    });
